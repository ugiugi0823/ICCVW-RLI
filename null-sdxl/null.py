...
def load_img(image_path, left=0, right=0, top=0, bottom=0):
...
class NullInversion:
    ...
    def next_step(self, model_output: Union[torch.FloatTensor, np.ndarray], timestep: int, sample: Union[torch.FloatTensor, np.ndarray]):
        ...
        # print(f"next_step_{next_timestep}")
    ...
    def null_optimization(self, latents, num_inner_steps, epsilon):
        ...
        for i in range(NUM_DDIM_STEPS):
            ...
            optimizer = Adam([uncond_embeddings, uncond_embeddings_p], lr=LR)
            ...
            for j in range(num_inner_steps):
                ...
                bar.set_description(f"Step {i}, Iteration {j}/{num_inner_steps}, Initial_loss : {initial_loss:.6f}, Target Loss {epsilon + i * 1e-5:.6f}, Loss: {loss_item:.6f}, Early stop : {loss_item < epsilon + i * 1e-5}")
    ...
    def invert(self, image_path: str, prompt: str,offsets = (0, 0, 0, 0), num_inner_steps=20, early_stop_epsilon=1e-5, verbose=False, do_1024=False):
    ...
    def __init__(self, model):
        # scheduler = DDIMScheduler(beta_start=0.0001, beta_end=0.02, beta_schedule="scaled_linear", clip_sample=False,
