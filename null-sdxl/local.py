...
from null import LOW_RESOURCE as LOW_RESOURCE_, NUM_DDIM_STEPS as NUM_DDIM_STEPS_, GUIDANCE_SCALE as GUIDANCE_SCALE_, MAX_NUM_WORDS as MAX_NUM_WORDS_
...
def save_tensor_images_pil_numpy(
    ...
    """
    PyTorch 텐서(배치 흑백 이미지)를 입력받아 PIL과 NumPy를 사용하여
    개별 이미지 파일로 저장합니다.

    Args:
        tensor (torch.Tensor): 입력 텐서. [B, 1, H, W] 형태의 4D 텐서여야 합니다.
                                (B: 배치 크기, 1: 채널(흑백), H: 높이, W: 너비)
        output_dir (str): 이미지를 저장할 디렉토리 경로.
        prefix (str, optional): 저장될 파일명의 접두사. Defaults to "image".
        input_range (Tuple[float, float], optional): 입력 텐서 값의 예상 범위.
                                                    (0, 1) 또는 (-1, 1)을 지원합니다.
                                                    Defaults to (0, 1).

    Raises:
        ValueError: 입력 텐서의 차원이나 채널 수가 적절하지 않거나,
                    지원하지 않는 input_range가 제공된 경우.
    """
    # --- 입력 유효성 검사 ---\
    # print("tensor",tensor)
    ...
    if tensor.ndim != 4:
        raise ValueError(f"Input tensor must be 4D (B, C, H, W), but got {tensor.ndim}D.")
    if tensor.shape[1] != 1:
        raise ValueError(f"Input tensor must have 1 channel (grayscale), but got {tensor.shape[1]} channels.")
    if input_range not in [(0, 1), (-1, 1)]:
        raise ValueError(f"Unsupported input_range: {input_range}. Supported ranges are (0, 1) and (-1, 1).")
...
class LocalBlend:
    ...
    def __call__(self, x_t, attention_store):
        ...
        if self.counter > self.start_blend:
            ...
            for idx, item in enumerate(attention_store["up_cross"][:36]+attention_store["down_cross"][:24]+attention_store["mid_cross"][:10]):
            ...
            maps_self = torch.cat(maps_self_list, dim=1) # torch.Size([2, 24, 1, 32, 32, 1024])
            maps_self_transpose = torch.cat(maps_self_transpose_list, dim=1) # torch.Size([2, 24, 1, 32, 32, 1024])
            ...
            mask_self_transpose_0 = mask_self_transpose[0:1]  # 첫 번째 요소만 선택 → shape: [1, 1, 64, 64]
            mask_self_transpose_fi = mask_self_transpose_0.repeat(2, 1, 1, 1)  # 복제 → shape: [2, 1, 64, 64]
    ...
    for i, t in enumerate(tqdm(model.scheduler.timesteps[-start_time:])):
        if uncond_embeddings_ is None:
            ...
            if uncond_embeddings[i].size() == prompt_embeds.size():
                context = torch.cat([uncond_embeddings[i].to(model.device), prompt_embeds])
                context_p = torch.cat([uncond_embeddings_p[i].to(model.device), pooled_prompt_embeds.to(model.device)]) 
            else:
                context = torch.cat([uncond_embeddings[i].repeat(2, 1, 1).to(model.device), prompt_embeds])
                context_p = torch.cat([uncond_embeddings_p[i].repeat(2, 1).to(model.device), pooled_prompt_embeds.to(model.device)]) 
        else:
...
def run_and_display(model,neg_prompts,prompts, controller, latent=None, run_baseline=False, generator=None, uncond_embeddings=None,uncond_embeddings_p=None,add_time_ids1=None ,verbose=True, steps=50):
    ...
    images, x_t = text2image_sdxl_lri(model, neg_prompts,prompts, controller, latent=latent, num_inference_steps=steps, guidance_scale=GUIDANCE_SCALE, generator=generator, uncond_embeddings=uncond_embeddings, uncond_embeddings_p=uncond_embeddings_p)
    if verbose:
        pass
