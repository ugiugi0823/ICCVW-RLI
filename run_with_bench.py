...
def put_alpha_channel(image: Image):
    ...
    # 2. 원본 그레이스케일 이미지를 RGB로 변환 (여기서는 기존 밝기를 유지)
    rgb = image.convert("RGB")

    # 3. RGB 이미지에 알파 채널을 추가하여 RGBA 이미지 생성
    im_rgba = rgb.copy()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pick_image', nargs = '+', type=str, default=["000000000000"]) # the editing methods that needed to run
    ...
    for key, value in editing_instruction.items():
        if value.get("editing_type_id") in edit_category_list:  # 문자열로 비교 (JSON에서는 보통 문자열임)
