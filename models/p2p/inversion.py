...
class NegativePromptInversion:
    ...
    def invert(self, image_gt, prompt, npi_interp=0.0, alpha=None, skip_register=False):
        """
...
class NullInversion:
    ...
    def invert(self, image_gt, prompt, guidance_scale, num_inner_steps=10, early_stop_epsilon=1e-5, alpha=None, skip_register=False):
        self.init_prompt(prompt)
        if not skip_register:
            register_attention_control(self.model, None, alpha=alpha)
...
class DirectInversion:
    ...
    def offset_calculate(self, latents, num_inner_steps, epsilon, guidance_scale):
        noise_loss_list = []
        print("self context shape",self.context.shape)
