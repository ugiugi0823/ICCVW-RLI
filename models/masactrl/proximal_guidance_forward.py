...
from models.masactrl.masactrl_utils import register_attention_editor_diffusers
...
def proximal_guidance_diffusion_step(
    model,
    controller,
    latents,
    context,
    ...
    if edit_stage and prox is not None:
        if prox == "l1":
            ...
            if quantile > 0:
                ...
                threshold = (
                    -quantile
                )  # if quantile is negative, use it as a fixed threshold
            score_delta -= score_delta.clamp(-threshold, threshold)
            score_delta = torch.where(
                score_delta > 0, score_delta - threshold, score_delta
            )
            score_delta = torch.where(
                score_delta < 0, score_delta + threshold, score_delta
            )
            if (recon_t > 0 and t < recon_t) or (recon_t < 0 and t > -recon_t):
                ...
                if dilate_mask > 0:
                    ...
                    mask_edit = dilate(
                        mask_edit.float(), kernel_size=2 * radius + 1, padding=radius
                    )
    ...
    if (
        mask_edit is not None
        and inversion_guidance
        and (recon_t > 0 and t < recon_t)
        or (recon_t < 0 and t > -recon_t)
    ):
        recon_mask = 1 - mask_edit
        latents = (
            latents
            - recon_lr
            * (latents - x_stars[len(x_stars) - i - 2].expand_as(latents))
            * recon_mask
        )
...
def proximal_guidance_forward(
    ...
    for i, t in enumerate(model.scheduler.timesteps):
        ...
        latents = proximal_guidance_diffusion_step(
            model,
            controller,
            latents,
            context,
