...
class MutualSelfAttentionControl(AttentionBase):
    ...
    def __init__(
        ...
        self.layer_idx = (
            ...
            else list(range(start_layer, self.total_layers))
        )
        self.step_idx = (
...
class MutualSelfAttentionControlUnion(MutualSelfAttentionControl):
    def __init__(
        ...
        """
        super().__init__(
            start_step, start_layer, layer_idx, step_idx, total_steps, model_type
        )
    ...
    def forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        # source image branch
        out_u_s = super().forward(
            qu_s, ku_s, vu_s, sim, attnu_s, is_cross, place_in_unet, num_heads, **kwargs
        )
        out_c_s = super().forward(
            qc_s, kc_s, vc_s, sim, attnc_s, is_cross, place_in_unet, num_heads, **kwargs
        )
...
class MutualSelfAttentionControlMaskAuto(MutualSelfAttentionControl):
    ...
    def forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        if self.self_attns_mask is not None:
            ...
            out_u_target = out_u_target_fg * spatial_mask + out_u_target_bg * (
                1 - spatial_mask
            )
            out_c_target = out_c_target_fg * spatial_mask + out_c_target_bg * (
                1 - spatial_mask
            )
...
class MasaCtrlEditor:
    def __init__(self, method_list, device, num_ddim_steps=50) -> None:
        ...
        self.scheduler2 = DDIMSchedulerDev(
            beta_start=0.00085,
            beta_end=0.012,
            beta_schedule="scaled_linear",
            clip_sample=False,
        ...
        self.model = MasaCtrlPipeline.from_pretrained(
            model_name, scheduler=self.scheduler1
        ).to(device)
        self.neg_model = MasaCtrlPipeline.from_pretrained(
            model_name, scheduler=self.scheduler2
        ).to(device)
    ...
    def edit_image_null_text_inversion_MasaCtrl(
        ...
        _, _, x_stars, uncond_embeddings = null_inversion.invert(
            image_gt=image_gt, prompt=prompt_src, guidance_scale=guidance_scale, skip_register=True
        )
        x_t = x_stars[-1]
        ...
        out_image = np.concatenate(
            (
                ...
                (image_gt),
                (
                    image_masactrl[0].permute(1, 2, 0).detach().cpu().numpy() * 255
                ).astype(np.uint8),
                (
                    image_masactrl[-1].permute(1, 2, 0).detach().cpu().numpy() * 255
                ).astype(np.uint8),
            ),
            1,
        )
    ...
    def edit_image_negative_prompt_inversion_MasaCtrl(
        ...
        editor = AttentionBase()
        register_attention_editor_diffusers(self.neg_model, editor, alpha=alpha)

        null_inversion = NegativePromptInversion(
            model=self.neg_model, num_ddim_steps=self.num_ddim_steps
        )
        
        _, image_enc_latent, x_stars, uncond_embeddings = null_inversion.invert(
            image_gt=image_gt, prompt=prompt_src, npi_interp=npi_interp, alpha=alpha, skip_register=True   
        )
        x_t = x_stars[-1]
        print("🎙️ inversion finished")
        ...
        #     model=self.model.vae, latents=reconstruct_latent
        # )[0]
        # hijack the attention module
        editor = MutualSelfAttentionControl(step, layper)
        register_attention_editor_diffusers(self.neg_model, editor, alpha=alpha)
        ...
        # inference the synthesized image
        image_masactrl = self.neg_model(prompts,
                            latents= x_t.expand(len(prompts), -1, -1, -1),
                            guidance_scale=guidance_scale,
                            uncond_embeddings=uncond_embeddings)

        print("🎙️ second sampling finished")
        # image_masactrl = latent2image(model=self.neg_model.vae, latents=latents_masactrl)
